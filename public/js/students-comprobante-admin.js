/**
 * Comprobante administrativo por estudiante
 */
(function () {
  const idEst = window._idEstudiante;
  const params = new URLSearchParams(window.location.search);

  function configurarLinkVolver() {
    const btnVolver = document.getElementById('btn-volver-perfil');
    if (!btnVolver) return;
    const idMatricula = params.get('id');
    btnVolver.href = idMatricula
      ? `/students/perfil-admin/${idEst}?id_matricula=${encodeURIComponent(idMatricula)}`
      : `/students/perfil-admin/${idEst}`;
  }
  ...
  async function cargar() {
    ...
    try {
      const idMatricula = params.get('id');
      const url = idMatricula
        ? `/api/enrollment/comprobante/${idMatricula}`
        : `/api/enrollment/comprobante/estudiante/${idEst}`;
      ...
      if (m.estado_pago !== 'Pagada' && parseFloat(m.saldo_factura || 0) > 0) {
        const botonGestion = document.getElementById('comp-pagar-btn');
        if (botonGestion) botonGestion.style.display = '';
      }
