 ...
 * Gestion de usuarios: listar, buscar, crear, editar, activar/desactivar
 */
(function () {
  ...
  async function cargar() {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--rojo-peligro);">Error: ${esc(e.message)}</td></tr>`;
    }
  }
        ...
        return `
          <tr>
            ...
            <td><div class="acciones-fila">
              ...
              ${u.activo
                ? `<button class="btn btn-sm" style="background:var(--rojo-peligro-bg);color:var(--rojo-peligro);" onclick="toggleUsuario(${u.id_usuario},0)">Desactivar</button>`
                : `<button class="btn btn-sm" style="background:var(--verde-bg);color:var(--verde-principal);" onclick="toggleUsuario(${u.id_usuario},1)">Activar</button>`}
            </div></td>
          </tr>`;
      }).join('');
    }

    U.renderPaginacion('#paginacion-usuarios', {
      total,
      page,
      limit: LIMIT,
  ...
  function nombreRolSeleccionado(selectEl) {
    return selectEl?.options[selectEl.selectedIndex]?.text || '';
  }

  function toggleCamposEstudiante(form) {
    if (!form) return;
    const rolSelect = form.querySelector('select[name=id_rol]');
    const isEst = /estudiante/i.test(nombreRolSeleccionado(rolSelect));
    form.querySelectorAll('.bloque-estudiante').forEach(el => {
      el.style.display = isEst ? '' : 'none';
    });
    const prog = form.querySelector('select[name=id_programa]');
    if (prog) prog.required = isEst;
  }
  ...
  async function cargarRoles() {
    try {
      ...
      document.querySelectorAll('select[name=id_rol]').forEach(sel => {
        sel.addEventListener('change', () => toggleCamposEstudiante(sel.closest('form')));
      });
    } catch (_) {}
  }
  ...
  async function cargarProgramas() {
    try {
      ...
      document.querySelectorAll('select[name=id_programa]').forEach(sel => {
        const valor = sel.value;
        sel.innerHTML = '<option value="">Seleccionar programa...</option>' +
          rows.map(p => `<option value="${p.id_programa}">${esc(p.nombre)}</option>`).join('');
        if (valor) sel.value = valor;
      });
    } catch (_) {}
  }
  ...
  formCrear?.addEventListener('submit', async e => {
    ...
    try {
      ...
      const rolTxt = nombreRolSeleccionado(formCrear.querySelector('select[name=id_rol]'));
      if (!/estudiante/i.test(rolTxt)) {
        delete body.id_programa;
        delete body.carne;
      }
      const { data } = await axios.post('/api/usuarios', body);
      if (!data.ok) throw new Error(data.error || 'No se pudo crear usuario');
      U.toast('Usuario creado correctamente', 'exito');
      U.closeModal('modal-crear');
      formCrear.reset();
      toggleCamposEstudiante(formCrear);
      await cargar();
    } catch (e) {
  ...
  window.editarUsuario = async function (id) {
    try {
      ...
      if (!data.ok) throw new Error(data.error || 'No se pudo cargar usuario');
      const u = data.data;
      ...
      if (form.querySelector('[name=id_programa]')) form.querySelector('[name=id_programa]').value = u.id_programa || '';
      if (form.querySelector('[name=carne]')) form.querySelector('[name=carne]').value = u.carne || '';
      toggleCamposEstudiante(form);
  ...
  formEditar?.addEventListener('submit', async e => {
    ...
    try {
      ...
      const rolTxt = nombreRolSeleccionado(formEditar.querySelector('select[name=id_rol]'));
      if (!/estudiante/i.test(rolTxt)) {
        delete body.id_programa;
        delete body.carne;
      }
      const { data } = await axios.put(`/api/usuarios/${id}`, body);
      if (!data.ok) throw new Error(data.error || 'No se pudo actualizar usuario');
  ...
  selEst?.addEventListener('change', e => {
    if (e.target.value === '1') estadoFiltro = 'Activo';
    else if (e.target.value === '0') estadoFiltro = 'Inactivo';
    else estadoFiltro = '';
    page = 1;
    cargar();
  });
  ...
  function checkAutoEditar() {
    ...
    if (idEditar && !isNaN(idEditar)) {
      editarUsuario(parseInt(idEditar, 10));
  ...
  (async function init() {
    ...
    toggleCamposEstudiante(formCrear);
    toggleCamposEstudiante(formEditar);
    await cargar();
    checkAutoEditar();
  })();
})();
